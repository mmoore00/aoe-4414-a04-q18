...
# Converts ECI reference frame to ECEF x, y, and z directions
#
# Parameters:
...
#  eci_x_km: x directional ECI value in kilometers
#  eci_y_km: y directional ECI value in kilometers
#  eci_z_km: z directional ECI value in kilometers
#  
# Output:
...
rECI = [eci_x_km, eci_y_km, eci_z_km]

for i in range(len(RzGMST)):
    for j in range(len(RzGMST[0])):
        rECEF[i] += RzGMST[i][j] * rECI[j]
print(rECEF[0])
print(rECEF[1])
print(rECEF[2])
